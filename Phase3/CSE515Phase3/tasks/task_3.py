...
def task3(first_image_folder, second_image_folder, feature_model, dimensionality_reduction, classifier='SVM',
          k=5):
    ...
    if database.check_if_latent_semantic_exist(X_train_id):
        ...
        Y_train = helper.get_image_labels(images_names, 3, "Training")
    else:
        ...
        Y_train = helper.get_image_labels(all_images, 3, "Training")
        X_train = reduction_algorithm_output[0]
        if dimensionality_reduction != "ALL":
            database.save_latent_semantics(first_image_folder + '-' + feature_model + '-' + dimensionality_reduction + '-'
                                       + classifier + '-' + str(k) + '-X_train', X_train)
            database.save_latent_semantics(first_image_folder + '-' + feature_model + '-' + dimensionality_reduction + '-'
                                       + classifier + '-' + str(k) + '-Y_train', Y_train)
    ...
    Y_test = helper.get_image_labels(test_images, 3, "Testing")
...
if __name__ == '__main__':
    ...
    task3(training_folder_path, testing_folder_path, model_name, dimensionality_reduction, image_classifier, k)
    # task3('/Users/keenan/Desktop/ASU/Semester 1/Multimedia and Web Databases/Project/Phase3/CSE515Phase3/train',
