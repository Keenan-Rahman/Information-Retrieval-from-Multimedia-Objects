...
# CSE 515
# Project Phase III
# This program applies the PCA dimensionality reduction technique to retrieve the latent
# semantics of a dataset ranked by data preservation
...
def pca(k, dataset):
    # PCA method. Input should be provided as integer k latent features to select and an n x p numpy
    # array where n is the number of data vectors and p is the numer of elements per data vector
    ...
    covarianceMatrix = ((np.dot(deviationsFromMean.T, deviationsFromMean)) / (deviationsFromMean.shape[
                                                                                  0] - 1))  # covariance is taken by taking the dot product of the transpose of the means deviated matrix with the regular matrix and then dividing it by n - 1
    ...
    return [projection, finalEVectors.T, sortedEVals,
            sortedEVectors]  # return a list containing the PCA-applied transformation of the input matrix as well as the selected bases vecotrs and the full set of sorted eigenvalues and eigenvectors.
