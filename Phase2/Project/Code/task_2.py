...
def task2(feature_model, Y, k, dimensionality_reduction):
    image_dictionary = database.get_image_data_by_subject(Y)
    ...
    database.save_latent_semantics_to_feature_matrix("image_weight_pair-Subject-"+str(Y)+"-"+
                                                     feature_model+"-"+str(k)+"-"+dimensionality_reduction,
                                                     data_frame)
    type_weight_pair = create_type_weight_pair(data_frame, Y, columns)
    filename = create_output_file(type_weight_pair, feature_model, Y, k, dimensionality_reduction)
...
def create_type_weight_pair(data_frame, Y, columns):
    types = ["cc", "con", "detail", "emboss", "jitter", "neg", "noise1", "noise2", "original", "poster", "rot",
             "smooth", "stipple"]
    type_weight_pair = pd.DataFrame(columns=columns)
    for type_name in types:
        sub_string = "image-" + type_name + "-" + str(Y) + "-"
        #temp_dataframe = data_frame[data_frame.index.str.startswith(sub_string)]
        temp_dataframe = data_frame[data_frame.index.str.contains(sub_string)]
        type_weight_pair.loc[type_name] = [value / len(temp_dataframe) for value in temp_dataframe.sum().values]
    return type_weight_pair.sort_values(by=columns, ascending=False)


def create_output_file(type_weight_pair, feature_model, Y, k, dimensionality_reduction):
    path = str(pathlib.Path().resolve()) + '/output/task_2'
    filename = feature_model + '-' + str(Y) + '-' + str(k) + '-' + dimensionality_reduction + '.csv'
    if not os.path.exists(path):
        os.makedirs(path)
    file = open(path + '/' + filename, 'w+')
    file.write(type_weight_pair.to_csv(index=True, header=True, sep=',', index_label='Subject-' + str(Y) + ' Types'))
    return filename


model_name = input("\nEnter Feature Model Names From\n- color_moment\n- local_binary_pattern\n- "
                   "histogram_of_oriented_gradients\n\nPlease Enter Model Name: ")
Y = input("\nPlease Enter Value of Y From 1-40: ")
k = int(input("\nPlease Enter K-Value: "))
dimensionality_reduction = input("\nEnter Dimensionality Reduction Model Names From\n- PCA\n- SVD\n- "
                                 "LDA\n- KMeans\n\nPlease Enter Model Name: ")

task2(model_name, Y, k, dimensionality_reduction)


#
# task2("local_binary_pattern",
#       4,
#       3,
#       "SVD")
